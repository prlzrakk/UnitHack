...
function getProjectFromUrl() {
    ...
    if (project && window.PROJECTS?.[project]) {
        return project;
    }
...
function getInitialTeams() {
    const project = window.PROJECTS?.[activeProjectId];

    const baseTeams = project?.teams?.length
        ? project.teams
        : [
    ...
    return baseTeams.map((team, index) => ({
        id: slugify(team.name) || `team-${index}`,
        name: team.name,
        color: team.color || "#f4864d",
        members: getDemoMembers(index),
    }));
}
